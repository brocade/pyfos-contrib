...
def online_in_name_server_by_alias(current_defined, ns_attributes, device_alias):
...
def online_in_name_server_by_dcommai(ns_attributes, device_dcommai):
    ...
    if isinstance(ns_attributes, list):
        for ns_attribute in ns_attributes:
            ...
            if domain == dev_domain and index == ns_attribute.peek_port_index():
                return True
    else:
...
def main(argv):
    ...
    if len(hanging_zones) is not 0:
        print("")
        user_input = input("Delete all hanging zones? (YES in all cap or anything else to skip) ")
        if user_input == "YES":
            ...
            if pyfos_util.is_failed_resp(result):
                print("Failed to delete. Aborting transaction", result)
                ...
                if pyfos_util.is_failed_resp(result):
                    print("Failed to aborting", result)
                else:
                    print("Aborted")
            else:
                ...
                result = new_effective.patch(session)
                if pyfos_util.is_failed_resp(result):
                    print("Failed to CFG Save. Aborting transaction", result)
                    new_effective = pyfos_zone.effective_configuration()
                    new_effective.set_cfg_action(pyfos_zone.CFG_ACTION_ABORT)
                    result = new_effective.patch(session)
                    if pyfos_util.is_failed_resp(result):
